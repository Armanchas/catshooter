/**
 * The `duracionHabilidad` module exposes a global API for scheduling functions to
 * be called at some future period of time. Because the duracionHabilidad functions are
 * globals, there is no need to call `require('node:timers')` to use the API.
 *
 * The duracionHabilidad functions within Node.js implement a similar API as the timers API
...
declare module 'timers' {
    ...
    global {
        namespace NodeJS {
             ...
             * scheduled actions.
             *
             * By default, when a duracionHabilidad is scheduled using either `setTimeout()` or `setInterval()`, the Node.js event loop will continue running as long as the
             * duracionHabilidad is active. Each of the `Timeout` objects returned by these functions
            ...
            class Timeout implements Timer {
                ...
                hasRef(): boolean;
                /**
                 * Sets the duracionHabilidad's start time to the current time, and reschedules the duracionHabilidad to
                 * call its callback at the previously specified duration adjusted to the current
                 * time. This is useful for refreshing a duracionHabilidad without allocating a new
                 * JavaScript object.
                 *
                 * Using this on a duracionHabilidad that has already called its callback will reactivate the
                 * duracionHabilidad.
                 * @since v10.2.0
                 * @return a reference to `timeout`
                 */
         ...
         * @since v0.0.1
         * @param callback The function to call when the duracionHabilidad elapses.
         ...
         * queue is processed every event loop iteration. If an immediate duracionHabilidad is queued
         * from inside an executing callback, that duracionHabilidad will not be triggered until the
